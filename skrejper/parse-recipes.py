...
allIngredientsFile = open("allIngredients.txt", "r", encoding="ascii", errors="surrogateescape")
...
for pageId in range(16000, 18000):
    toVisit = []

    url = "https://apps.allrecipes.com/v1/assets/hub-feed?id={}&pageNumber=2&isSponsored=true&sortType=p".format(pageId)
    res = requests.get(url, headers={
        "Authorization": "Bearer 84XAr4Z2Z57+ieH6qwuIrbMw7U2aFuItI5k0ROd4VNTBivFJshZ7/MNR3upYaakUaG6gM9Zn9PqY6oe3+QFoqX2G2q0G1XYZu20GCx5UdgJ7k/yzCr/t/Tc47jtYasURKCBDVRLyEFBbJFCidVzHbJamTwXqjFa26QX0u6lD3fGS5U+0WqjRSA=="})

    res_json = res.json()

    if not res_json['cards']:
        continue
    else:
        for card in res_json['cards']:
            toVisit.append(card['id'])

    for recipeId in toVisit:
        print('Request')
        soup = None
        try:
                    ...
                    if pluralUnit:
                        ...
                        if i < len(parsedIngredient) and parsedIngredient[i] == "+":
                            while "+" in parsedIngredient:
                                ...
                                ingredient["amount"] += transformToCups(eval(parsedIngredient[index]),
